...
import controller.RandomSolver;
import controller.Solver;
import controller.StabilStudentSolver;
...
public class Main {
    private static Problem recreateExample() {
        ...
        // Creating with new ArrayList because only letting Arrays.asList(..), that would make the list unmodifiable
        // (which we don't want)
        students[0] = new Student("S1", "", "1@google.com", new ArrayList<>(Arrays.asList(projects[0], projects[6])), null);
        students[1] = new Student("S2", "", "2@google.com", new ArrayList<>(Arrays.asList(projects[0], projects[1], projects[2],
                projects[3], projects[4], projects[5])), null);
        students[2] = new Student("S3", "", "3@google.com", new ArrayList<>(Arrays.asList(projects[1], projects[0], projects[3])),
                null);
        students[3] = new Student("S4", "", "4@google.com", new ArrayList<>(Collections.singletonList(projects[1])), null);
        students[4] = new Student("S5", "", "5@google.com", new ArrayList<>(Arrays.asList(projects[0], projects[1], projects[2],
                projects[3])), null);
        students[5] = new Student("S6", "", "6@google.com", new ArrayList<>(Arrays.asList(projects[1], projects[2], projects[3],
                projects[4], projects[5])), null);
        students[6] = new Student("S7", "", "7@google.com", new ArrayList<>(Arrays.asList(projects[4], projects[2], projects[7])), null);
    ...
    public static void main(String[] args) {
        ...
        problem = recreateExample();
        Solver randomSolver = new RandomSolver(problem);
        randomSolver.solve();
